...
#ifndef _NONCE_STEP_SIZE_LOCATION
#define _NONCE_STEP_SIZE_LOCATION \
  1  // 0 = shared mem, >=1 = registers
#endif
...
typedef struct __align__(4 * sizeof(WORD)) uint128_t {
    WORD _0;
    WORD _1;
    WORD _2;
    WORD _3;
...
extern "C" __global__ void mine_sha256(const BYTE const_in[SHA256_BLOCK_SIZE],
                                       BYTE nonce_out[sizeof(uint128_t)],
                                       const WORD nonce_step_size,
                                       const WORD n_batch_device,
                                       const BYTE max_valid_hash[SHA256_BLOCK_SIZE],
                                       const BYTE init_nonce[sizeof(uint128_t)]) {
...
#pragma unroll(SHA256_BLOCK_SIZE / sizeof(WORD))
...
#if _NONCE_STEP_SIZE_LOCATION == 0  // shared mem
    __shared__ uint128_t _nonce_step_size;
    if (threadIdx.x == 0) _nonce_step_size._0 = nonce_step_size;
#else
    uint128_t _nonce_step_size;
    _nonce_step_size._0 = nonce_step_size;
#endif
    ...
    {
        WORD carry = 0;
        nonce = *((uint128_t *) init_nonce);
        nonce._0 += thread;
        carry = nonce._0 < ((WORD *) init_nonce)[0];
        for (int i = 1; i < sizeof(uint128_t) / sizeof(WORD); i++) {
            ((WORD *) &nonce)[i] += carry;
            carry = ((WORD *) &nonce)[i] < ((WORD *) init_nonce)[i];
        }
    }
