...
_cuda_mining_kernel: "cp.RawKernel" = None


class CudaLaunchConfig:
    def __init__(self, n_blocks: int = 12, n_threads: int = 1024):
        assert 1024 >= n_threads > 0, "n_threads must be positive"
        assert n_blocks > 0, "n_blocks must be positive"
        self.n_blocks = n_blocks
        self.n_threads = n_threads


CUDA_LAUNCH_CONFIG = CudaLaunchConfig()


def load_cuda_miner(path="miner.cu"):
    import cupy as cp
    global _cuda_mining_kernel, cp
    with open(path) as f:
        cuda_miner_code = f.read()
    _cuda_mining_kernel = cp.RawModule(code=cuda_miner_code).get_function("mine_sha256")
...
def serialize_balances(balances: dict[bytes, int]) -> bytes:
    return b''.join(k + v.to_bytes(BALANCE_SERIALIZATION_VALUE_SIZE, ENDIAN) for k, v in balances.items())


def deserialize_balances(raw: bytes) -> dict[bytes, int]:
    return {raw[i:i + KEY_SIZE // 8]: int.from_bytes(
        raw[i + KEY_SIZE // 8:i + KEY_SIZE // 8 + BALANCE_SERIALIZATION_VALUE_SIZE], ENDIAN) for i in
        range(0, len(raw), KEY_SIZE // 8 + BALANCE_SERIALIZATION_VALUE_SIZE)}
...
class Transaction:
    ...
    def __init__(self, sender: bytes, receiver: bytes, amount: int, fee: int, uuid: bytes,
                 signature: bytes):
        self.sender = sender
    ...
    def is_sane(self, const_transaction_fee: int, relative_transaction_fee: float):
        ...
        correct_fee = self.fee == const_transaction_fee + int(self.amount * relative_transaction_fee)
        ...
        conditions = (positive, correct_fee, amount_exceeds_fee, sender_not_receiver, signature_valid)
        return all(conditions)

    def is_valid(self, balances: dict[to_bytes, int], invalidated_uuids: set[to_bytes], const_transaction_fee: int,
                 relative_transaction_fee: float, transaction_sane: bool = None):
        can_afford = balances.get(self.sender, 0) > self.amount > 0
        uuid_valid = self.uuid not in invalidated_uuids
        is_sane = self.is_sane(const_transaction_fee,
                               relative_transaction_fee) if transaction_sane is None else transaction_sane
...
class Block:
    SIZE = 4
    BYTE_COUNTS = [8, 32, 512] + [Transaction.N_BYTES] * SIZE
    N_BYTES = sum(BYTE_COUNTS)
    CONVERSION_F = [lambda b: int.from_bytes(b, ENDIAN), LXX, LXX] + [Transaction.from_bytes] * SIZE

    def __init__(self, transactions: list["Transaction"], prev: "Block", validator: bytes, nonce: int = 0):
    ...
    def to_bytes(self):
        return self.nonce.to_bytes(Block.BYTE_COUNTS[0], ENDIAN) + self._bytes_without_val()
    ...
    def from_bytes(cls, raw: to_bytes):
        ...
        transactions, nonce, prev, validator = components
        return cls(transactions, prev, validator, nonce)
    ...
    def hash(self):
        if self._cache_hash is None or self.nonce != self._cache_val:
            ...
            self._cache_hash = sha256(self.nonce.to_bytes(Block.BYTE_COUNTS[0], ENDIAN) + self._cache_inner_hash)
...
class NonceMiningJobHandler:
    ...
    def launch_job(self, block: "Block", use_cuda_miner: bool, valid_block_max_hash: int):
    ...
    def _worker(self):
        while not self._killed:
            ...
            if block.nonce != 0:
                raise ValueError("nonce already set to non-zero value, meaning it was already mined")
            elif use_cuda_miner:
                if cp is None and _cuda_mining_kernel is None:
                    raise RuntimeError("CUDA miner not loaded; you probably forgot to call load_cuda_miner")
                elif _cuda_mining_kernel is None:
                    raise RuntimeError("CUDA miner kernel not loaded (but cupy is)")
                elif cp is None:
                    raise RuntimeError("cupy is not loaded (but CUDA miner kernel is)")
                h = int.from_bytes(block.hash(), ENDIAN)
                const_in = cp.empty((4,), dtype=cp.ulonglong)
                max_valid_hash = cp.empty_like(const_in)
                nonce_out = cp.zeros((1,), dtype=cp.ulonglong)
                ...
                n_batch = CUDA_LAUNCH_CONFIG.n_blocks * CUDA_LAUNCH_CONFIG.n_threads
                args = (const_in, nonce_out, n_batch, n_batch, max_valid_hash, 1)
                _cuda_mining_kernel((CUDA_LAUNCH_CONFIG.n_blocks,), (CUDA_LAUNCH_CONFIG.n_threads,), args)
                while not self._killed and nonce_out[0].item() == 0:
                    thread_yield()
                if not self._killed:
                    cp.cuda.runtime.deviceSynchronize()
                    block.nonce = nonce_out[0].item()
            else:
                block_ = Block(block.transactions, block.prev, block.validator, 1)
                while not self._killed and not is_valid_block(block_, valid_block_max_hash):
                    block_.nonce += 1
                if not self._killed:
...
class OpenBlock:
    ...
    def receive(self, transaction: "Transaction", balances: dict[bytes, int], invalidated_uuids: set[bytes],
                const_transaction_fee: int, relative_transaction_fee: float, val_reward: int,
                transaction_sane: bool = None) -> tuple[bool, bool]:
        if not transaction.is_valid(balances, invalidated_uuids, const_transaction_fee, relative_transaction_fee,
                                    transaction_sane):
            return False, False
...
def broadcast_balances(broadcast_buffer, balances: dict[bytes, int], broadcast_copy: bool = False,
...
def _mine(validator: bytes, chain: "Block" = None, transaction_recv_channel=None, chain_recv_channel=None,
          chain_broadcast_channel=None, balance_broadcast_channel=None, side_channel=None, init_balance=None,
          terminate_event=None, has_terminated_event=None, use_cuda_miner_event=None,
          incompatible_chain_distrust: int = 1, compatible_chain_distrust: int = 1, val_reward: int = 0,
          const_transaction_fee: int = 0, relative_transaction_fee: float = 0.0, max_recv_chains_per_iter: int = -1,
          max_recv_transactions_per_iter: int = -1, collect_time: int = 0, copy_balances_on_broadcast=False,
          transaction_backlog_size: int = 1, broadcast_balances_as_bytes: bool = False,
    ...
    assert max_recv_transactions_per_iter != 0, "max_recv_transactions_per_iter cannot be 0"
    assert collect_time >= 0, "collect time cannot be negative"
    assert incompatible_chain_distrust > 0, "an external chain cannot be trusted as much as or more than more than your own"
    assert compatible_chain_distrust > 0, "an external chain cannot be trusted as much as or more than more than your own"
    ...
    assert chain_broadcast_channel is None or isinstance(chain_broadcast_channel, (queue.Queue, mp.queues.Queue, list))
    assert balance_broadcast_channel is None or isinstance(balance_broadcast_channel,
                                                           (queue.Queue, mp.queues.Queue, list))
    assert side_channel is None or isinstance(side_channel, (queue.Queue, mp.queues.Queue, list))
    assert init_balance is None or isinstance(init_balance, dict)
    assert terminate_event is None or isinstance(terminate_event, (threading.Event, mp.synchronize.Event))
    assert has_terminated_event is None or isinstance(has_terminated_event, (threading.Event, mp.synchronize.Event))
    assert use_cuda_miner_event is None or isinstance(use_cuda_miner_event, (threading.Event, mp.synchronize.Event))
    ...
    while terminate_event is None or not terminate_event.is_set():
        ...
        for recv_chain in recv_chains:
            ...
            if is_valid and (new_chain_len - current_chain_len - incompatible_chain_distrust + 1 > max_trust_level or (
                    new_chain_len - current_chain_len - compatible_chain_distrust + 1 > max_trust_level and new_balances == balances)):
                favorite_new_chain_data = (recv_chain, new_balances, new_invalidated_uuids)
                max_trust_level = max(new_chain_len - current_chain_len - incompatible_chain_distrust + 1,
        ...
        recv_trans = get_received_transactions(transaction_recv_channel, max_recv_transactions_per_iter)
        t_is_sane = [t.is_sane(const_transaction_fee, relative_transaction_fee) for t in recv_trans]
        if side_channel is not None:
            for t, sane in zip(recv_trans, t_is_sane):
                if not sane:
                    broadcast_side_channel(side_channel, SideChannelItemType.TRANSACTION_VALIDITY_PAIR,
                                           (t.to_bytes(), False))
        filtered_recv_transactions = [t for t, sane in zip(recv_trans, t_is_sane) if sane]
        recv_sane_transactions = sorted(filtered_recv_transactions, key=lambda t: t.amount, reverse=True)
        ...
        for t in transactions:
            ...
            is_closed, is_valid_t = open_block.receive(t, balances, invalidated_uuids, const_transaction_fee,
                                                       relative_transaction_fee, val_reward, True)
            if side_channel is not None:
                broadcast_side_channel(side_channel, SideChannelItemType.TRANSACTION_VALIDITY_PAIR,
            ...
            if is_closed:
                ...
                sorted_transactions = sorted(open_block.transactions, key=lambda t: t.amount, reverse=True)
                chain = Block(sorted_transactions, chain, validator)
                mining_job_handler.launch_job(chain, use_cuda_miner_event is not None and use_cuda_miner_event.is_set(),
        ...
        if updated_chain and chain_is_ready(chain):
            ...
            broadcast_balances(balance_broadcast_channel, balances, copy_balances_on_broadcast,
...
class MiningConfig:
    def __init__(self, incompatible_chain_distrust: int = 1, compatible_chain_distrust: int = 1, val_reward: int = 0,
                 const_transaction_fee: int = 0, relative_transaction_fee: float = 0.0,
                 max_recv_chains_per_iter: int = 1, max_recv_transactions_per_iter: int = 4, collect_time: int = 0,
                 copy_balances_on_broadcast: bool = True, transaction_backlog_size: int = 2,
...
class Miner:
    ...
    def __init__(self, validator: bytes, chain: "Block" = None, init_balance=None, config: MiningConfig = None,
                 run_with: str = "direct"):
        super().__init__()
        self.validator = validator
        self.chain = chain
        ...
        args = (self.validator, self.chain, self.transaction_recv_buffer, self.chain_recv_buffer,
                self.chain_broadcast_buffer, self.balance_broadcast_buffer, self.side_channel, self.init_balance,
                self.terminate_event, self.has_terminated_event, self.use_cuda_miner_event,
                self.incompatible_chain_distrust, self.compatible_chain_distrust, self.val_reward,
                self.const_transaction_fee, self.relative_transaction_fee, self.max_recv_chains_per_iter,
                self.max_recv_transactions_per_iter, self.collect_time, self.copy_balances_on_broadcast,
                self.transaction_backlog_size, self.broadcast_balances_as_bytes)
        if run_with == "direct":
            _mine(*args)
            ...
            raise ValueError("invalid run_with value: run_with must be one of ('direct', 'threading', 'mp')")
    ...
    def finish(self) -> tuple[Optional[bytes], Optional[Union[dict[bytes, int], bytes]]]:
