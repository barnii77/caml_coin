...
USE_CUDA = True
if USE_CUDA:
    blockchain.load_cuda_miner("cuda_miner_templates/nonce128_emulated_uint128_anywhere.cu", autotune=False, show_progress=True)
    print("CUDA miner loaded with config:", blockchain._cuda_miner_fixed_launch_config_kernel.launch_config)
...
def test_blockchain_class1():
    ...
    chain_handler = blockchain.Miner(
        ...
        blockchain.MiningConfig(
            valid_block_max_hash=0x0000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
        ),
        "threading",
    )
    if USE_CUDA:
